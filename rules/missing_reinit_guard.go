...
func (MissingReinitGuard) Check(c *rule.Context) []rule.Finding {
	...
	for _, fn := range c.ContractFns() {
		if fn.Name == "__constructor" || hasReinitGuard(fn.Body) {
			continue
		}
...
// predicates.
func hasReinitGuard(body rule.Node) bool {
	found := false
	body.Walk(func(n rule.Node) bool {
...
func guardCondition(condition rule.Node) bool {
	...
	if call, ok := rule.AsMethodCall(condition); ok && call.Name == "has" && receiverHasStorage(call.Recv) {
		return true
	}
	// get(&DataKey::X).is_some() — the positive alternative
	if call, ok := rule.AsMethodCall(condition); ok && call.Name == "is_some" && receiverHasStorage(call.Recv) {
		return true
	}
	...
	if condition.Kind() == "unary_expression" {
		...
		if ok {
			if call, ok := rule.AsMethodCall(inner); ok && call.Name == "is_none" && receiverHasStorage(call.Recv) {
				return true
			}
		}
	}
